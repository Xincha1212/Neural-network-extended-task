...
# 读取train_list.txt文件
data_list_file = "Cat12Data/train_list.txt"
with open(data_list_file, "r") as f:
    lines = f.readlines()

file_paths, labels = zip(*[line.strip().split() for line in lines])  # 将文件路径和标签分开
labels = [int(label) for label in labels]
file_paths = ['Cat12Data/' + file_path for file_path in file_paths]
...
for epoch in range(num_epochs):
    ...
    for real_images, labels in train_loader:
        ...
        optimizer_d.step()
        acc_real = correct_real / cnt
        acc_real = acc_real / batch_size
        print(f'Step: {stp}, Loss: {real_loss.item():.4f}, Accuracy: {acc_real:.4f}')
    ...
    # 保存模型参数
    checkpoint_path = f'Cat12Data/GAN_checkpoint/model_epoch_{epoch + 1}.pth'
    torch.save({
        'epoch': epoch,
