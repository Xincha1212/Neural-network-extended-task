...
if os.path.exists('NewsData/vocab.pkl'):
    vocab = pkl.load(open('NewsData/vocab.pkl', 'rb'))
else:
    vocab = build_vocab('NewsData/train.txt', tokenizer=tokenizer, max_size=MAX_VOCAB_SIZE, min_freq=1)
    pkl.dump(vocab, open('NewsData/vocab.pkl', 'wb'))
class_list = [x.strip() for x in open('NewsData/class.txt', encoding='utf-8').readlines()]
...
train_data = load_dataset('NewsData/train.txt', 32)
dev_data = load_dataset('NewsData/dev.txt', 32)
test_data = load_dataset('NewsData/test.txt', 32)
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch % 100 == 0:
                ...
                if dev_loss < dev_best_loss:
                    ...
                    torch.save(discriminator.state_dict(), 'NewsData/GAN.ckpt')
                    improve = '*'
...
def model_test(discriminator, generator, test_iter):
    # test
    discriminator.load_state_dict(torch.load('NewsData/GAN.ckpt'))
    discriminator.eval()
    start_time = time.time()
